...
//        }
    ...
    public void onReceive(Object message) throws Exception {
        if(message instanceof ClusterEvent.CurrentClusterState) {
            ...
            mediator.tell(new DistributedPubSubMediator.SendToAll(CONSUMER_ACTOR_PATH,Protocol.SHAKE_HANDS_ALL),getSender());
        }else{
    ...
    private void registerConsumer(Member member) {
        ...
        for(ActorRef ref: refs){
            getContext().actorSelection(member.address() + CONSUMER_ACTOR_PATH).tell("register",ref);
        }
